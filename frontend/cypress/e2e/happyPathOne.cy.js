describe('happy path one as decribed in the spec', () => {
  ...
  it('successful register', () => {
    ...
    cy.get('input[name=register-password-confirm]')
      .focus()
      .type('test123')

    cy.get('#register-submit-btn')
      .click()
    cy.url().should('include', 'localhost:3000');
  ...
  it('successfully create listing', () => {
    ...
    cy.get('#nav-yourListings-btn')
      .click()
    cy.url().should('include', 'localhost:3000/yourListings');


    cy.get('#create-listing-btn')
      .click()
    ...
    cy.get('input[name=thumbail]')
      .selectFile('src/images/thumbnail.jfif')
